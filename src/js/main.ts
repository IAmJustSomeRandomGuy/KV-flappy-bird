...
type Ability = "dashing" | "teleport" | "time freeze";
let ABILITIES: Ability[] = ["dashing", "teleport", "time freeze"];
type ButterflyState = "normal" | "jumping" | Ability;
class Butterfly {
  ...
  ABILITY_COOLDOWNS: Record<Ability, number> = {
    dashing: 2000,
    teleport: 5000,
    "time freeze": 9000,
  };
  ABILITY_DURATION: Record<Ability, number> = {
    dashing: 350,
    teleport: 200,
    "time freeze": 7000,
  };
  ...
  change_state(new_state: ButterflyState) {
    // @ts-ignore
...
function key_pressed(e: KeyboardEvent) {
  if (!playing) {
    start();
    return;
  }
...
class Scale {
  ...
  static SCALE_ELEMENTS = {
    bottom: document.getElementsByClassName("scale-bottom")[0] as HTMLElement,
    top: document.getElementsByClassName("scale-top")[0] as HTMLElement,
  };
  elements = {
    bottom: Scale.SCALE_ELEMENTS.bottom.cloneNode() as HTMLElement,
    top: Scale.SCALE_ELEMENTS.top.cloneNode() as HTMLElement,
  };
  ...
  process(delta: number) {
    ...
    if (
      this.pos.x + Scale.COLLISION_MARGINS.x < butterfly.pos.x &&
      this.pos.x + Scale.WIDTH - Scale.COLLISION_MARGINS.x > butterfly.pos.x - butterfly.SIZE
    ) {
      if (
        this.pos.y - Scale.COLLISION_MARGINS.y > butterfly.pos.y ||
...
class Flower {
  ...
  element = Flower.ELEMENTS[Math.floor(Math.random() * 6)]?.cloneNode() as HTMLElement;
...
function load() {
  ...
  console.log(values);
  if (values.highscore !== null) highscore = parseInt(values.highscore);
  if (values.flower_amount !== null) flower_amount = parseInt(values.flower_amount);
